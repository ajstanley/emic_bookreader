...
BookReader.prototype.init = function() {
    ...
    $(window).bind('resize', this, function(e) {
        ...
        if (this.constMode1up == e.data.mode) {
            ...
            e.data.loadLeafs();
        } else if (this.constModeThumb == e.data.mode){
...
BookReader.prototype.drawLeafsOnePage = function() {
    ...
    for (i=0; i<indicesToDisplay.length; i++) {
        ...
        if (BookReader.util.notInArray(indicesToDisplay[i], this.displayedIndices)) {
            ...
            //url = this._getPageURI(index, this.reduce, 0);
            ...
            //       }
            //       error: function(){
            //         img.src = url.replace("/JP2/", "/TIFF/");
            //       }
            //});

            var img = document.createElement("img");
            // a 404 error here most often means that djatoka could not process the jp2 because its too small
            // so fallback to djatoka'ing the tiff->png
            $(img).error(function() {
                var alt = $(this).attr("src").replace("/JP2/", "/TIFF/");
...
BookReader.prototype.lazyLoadImage = function (dummyImage) {
    ...
    $(img).error(function() {
        //var alt = $(this).attr("src").replace(br.djatoka_prefix, "").replace(/\/&.*/, "");
        var alt = $(this).attr("src").replace("/JP2/", "/TIFF/")
        //alert(alt, $(this).attr("src"));
        $(this).unbind("error").attr({
            src: alt
...
BookReader.prototype.prefetchImg = function(index) {
    ...
    if (loadImage) {
        ...
        // this successfully catches the 404 when the image is broken and replaces it with the
        // stock jp2 from underneath - **BUT** the jp2 is unreadable by most browsers and still
...
BookReader.prototype.jumpIndexForLeftEdgePageX = function(pageX) {
    ...
    if ('rl' != this.pageProgression) {
        ...
        return BookReader.util.clamp(Math.round(jumpIndex), this.firstDisplayableIndex(), this.twoPage.currentIndexL - 2);
    } else {
        jumpIndex = this.twoPage.currentIndexL + ($(this.leafEdgeL).offset().left + $(this.leafEdgeL).width() - pageX) * 10;
        return BookReader.util.clamp(Math.round(jumpIndex), this.twoPage.currentIndexL + 2, this.lastDisplayableIndex());
    }
}
...
BookReader.prototype.jumpIndexForRightEdgePageX = function(pageX) {
    ...
    if ('rl' != this.pageProgression) {
        // LTR
        jumpIndex = this.twoPage.currentIndexR + (pageX - $(this.leafEdgeR).offset().left) * 10;
        return BookReader.util.clamp(Math.round(jumpIndex), this.twoPage.currentIndexR + 2, this.lastDisplayableIndex());
    } else {
        jumpIndex = this.twoPage.currentIndexR - (pageX - $(this.leafEdgeR).offset().left) * 10;
        return BookReader.util.clamp(Math.round(jumpIndex), this.firstDisplayableIndex(), this.twoPage.currentIndexR - 2);
    }
}
...
BookReader.prototype.swipeMousemoveHandler = function(event) {
    ...
    var minSwipeLength = $('#BookReader').width()*4;//Math.min($('#BookReader').width() / 5, 80);
    var maxSwipeTime = 0;
...
BookReader.prototype._getPageURI = function(index, reduce, rotate) {
    ...
    if ('undefined' == typeof(reduce)) {
        ...
        //    scale = 32;
        //}
        else { // less branching is good, log() is not - torn
            scale = 1<<(Math.floor(Math.log(scale)/Math.log(2.0)));
        }
        reduce = scale;
    }
...
*/
            ...
            '</fieldset>',
/* // Jason: took this out since it says "not supported"
            '<fieldset>',
...
/* // Jason: should not be needed now that BRframEmbed no longer exists
